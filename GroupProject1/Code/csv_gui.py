    ...
    try:
        with open(file_path, 'a', newline='', encoding='utf-8') as csvfile:
            ...
            for line in data_list:
                # Split each line by comma to create a list
                row = line.split(",")
...
def submit_click():
    ...
    if input_list:
        ...
        # Clear the Text widget after appending data
...
def reorganize_button_click():
    ...
    with open(csv_file_path, mode='r', newline='', encoding="utf-8") as file:
        ...
        headers = next(csv_reader)  # Skip the headers
        data = list(csv_reader)

    # Sort the data based on the values in the first column (assuming they are strings)
    data.sort(key=lambda row: row[0])

    # Overwrite the original CSV file with the sorted data, including headers
